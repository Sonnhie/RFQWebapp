    ...
    function InitializeDataTable(){
        const renderTable = (data = [])=>{
            ...
            data.forEach(item => {
                    ...
                    const statusBadge = `<span class="status-badge ${statusClasses[item.item_status] || ''}">${item.item_status}</span>`;
                    const isDisabled = (item.item_remarks === 'Comparison approved by Requestor')
                    ...
                    const approveButton = `<button class="btn btn-sm btn-success rounded-2 me-3" 
                                            data-section="${item.item_section}" 
                                            data-id="${item.control_number}" 
                                            id="approve_btn" ${isDisabled ? 'disabled' : ''}>
                    ...
                    if ($role === 'Verifier-Approver' || $role === 'Requestor' || $role === 'Manager') {
            ...
            data.forEach(item =>{
                ...
                $tbodySupplier.append(`<tr id="rowIdx" data=>
                                ...
                                <td>
                                    ...
                                    <input type="text" id="item_supplier" name="supplier_name[]" data-index="${index}" value="${supplier.supplier_name}" class="form-control" />
                                </td>
                                <td>
                                    <input type="number" id="item_price" name="item_price[]" data-index="${index}" value="${parseFloat(supplier.item_price).toFixed(2)}" class="form-control" step="0.01" />
                                </td>
                                <td>
                                    <input type="number" id="item_discount" name="item_discount[]" data-index="${index}" value="${parseFloat(supplier.item_discount).toFixed(2)}" class="form-control" step="0.01" />
                                </td>
                                <td>
                                    <input type="number" id="item_total" name="item_total[]" data-index="${index}" value="${parseFloat(supplier.item_total).toFixed(2)}" class="form-control" step="0.01" readonly />
                                </td>
                        </tr>`);

                        setTimeout(() => {
                            $($tbodySupplier).off('input', '#item_price, #item_discount').on('input', '#item_price, #item_discount', function () {
            ...
            Swal.fire({
                    ...
                    if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: './backend/Route/requestRouteAction.php',
                                type: 'POST',
                                data: $($editForm).serialize()+'&action=' + action+ '&section=' + section + '&main=' + mainaddress + '&control_number=' + control_number,
                                dataType: 'json',
                                success: function(response){
                                    console.log(response);
                                    Swal.close();
